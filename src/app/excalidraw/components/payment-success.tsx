...
export default function PaymentSuccess() {
  const { storage, currentUser, setRoute } = useUserStore();
  ...
  return (
    <div className="p-4 flex h-full justify-between flex-col">
      <div>
        ...
        <div className="text-[20px] font-bold mt-8">Thank you!</div>
        <div className="text-[20px] font-bold mt-2">{currentUser?.name}</div>
        <div className="text-[20px] font-bold mt-8">You've been granted unlimited access</div>
        <Input
          disabled
          className="mt-3 w-full"
          value={storage?.license?.licenseKey?.slice(0, 10) + '**********' + storage?.license?.licenseKey?.slice(20)}
        />
      </div>
